...
const MyProjects = () => {
  ...
  const fetchProjects = async () => {
    try {
      setLoading(true);
      const { data } = await api.get("/api/user/projects"); // ✅ Correct endpoint
  ...
  const deleteProject = async (projectId: string) => {
    try {
      ...
      await api.delete(`/api/projects/${projectId}`); // ✅ Matches projectRountes.ts
      ...
      <div className="px-4 md:px-16 lg:px-24 xl:px-32">
        {loading ? (
          <div className="flex items-center justify-center h-[80vh]">
            <Loader2Icon className="w-8 h-8 animate-spin text-indigo-200" />
          </div>
        ) : projects.length > 0 ? (
          <div className="py-10 min-h-[80vh]">
            ...
            <div className="flex items-center justify-between mb-12">
              ...
              <button
                onClick={() => navigate("/")}
                className="flex items-center gap-2 text-white px-3 sm:px-6 py-2 rounded bg-gradient-to-br from-indigo-500 to-indigo-600 hover:opacity-90 active:scale-95 transition-all"
              >
                ...
                >
                  ...
                  <div className="p-4 text-white bg-gradient-to-b from-transparent group-hover:from-indigo-950 to-transparent transition-colors">
                    <div className="flex items-start justify-between">
                      ...
                      <button className="px-2.5 py-0.5 mt-1 ml-2 text-xs bg-gray-800 border border-gray-700 rounded">
                        Website
                      </button>
                    </div>
                  ...
                  >
                    <Trash2Icon className="absolute top-3 right-3 scale-100 bg-white p-1.5 size-7 rounded text-red-500 cursor-pointer transition-all" />
                  </div>
                </div>
              ))}
            </div>
