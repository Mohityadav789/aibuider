...
const Sidebar = ({isMenuOpen, project, setProject, isGenerating, setIsGenerating}: SidebarProps) => {
    ...
    const fetchProject = async () => {
        try {
            const { data } = await api.get(`/api/projects/preview/${project.id}`);
    ...
    try {
        ...
        await api.get(`/api/projects/rollback/${project.id}/${versionId}`);

        // ✅ fetch updated project AFTER rollback
    ...
    //         const updated = await api.get(`/api/project/preview/${project.id}`);
  ...
  try {
    ...
    const response = await api.post(
      `/api/projects/revision/${project.id}`,
      { message: input }
    );

    // ✅ Fetch updated project
    const updatedRes = await api.get(`/api/projects/preview/${project.id}`);
    const updatedProject = updatedRes.data?.project;

    if (!updatedProject) {
      throw new Error("Project not found in response");
    }

    // ✅ SAFE UPDATE (NO TYPE ERROR)
    setProject({
      ...project,
      ...updatedProject,
      conversation: updatedProject.conversation || [],
      versions: updatedProject.versions || [],
      current_code: updatedProject.current_code || project.current_code || "",
    });

    toast.success(response.data?.message || "Changes applied");
    setInput("");
